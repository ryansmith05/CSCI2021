...
el_blockhead_t *el_get_header(el_blockfoot_t *foot) {
    ...
    el_blockhead_t *header = PTR_PLUS_BYTES(foot, -(sizeof(el_blockhead_t) + size));
    return header;
}
...
el_blockhead_t *el_block_below(el_blockhead_t *block) {
    if (block == NULL || (void *)block <= el_ctl.heap_start) {
        return NULL;
    }
    ...
    if ((void *)below_header < el_ctl.heap_start) {
        return NULL;
    }
    return below_header;
}
...
el_blockhead_t *el_find_best_avail(size_t size) {
    el_blockhead_t *current = el_ctl.avail->beg->next;
    el_blockhead_t *best_avail = NULL;
    while (current != el_ctl.avail->end) {
        if (current->size >= size + EL_BLOCK_OVERHEAD &&
            (best_avail == NULL || current->size < best_avail->size)) {
            best_avail = current;
        }
        current = current->next;
    }
...
el_blockhead_t *el_split_block(el_blockhead_t *block, size_t new_size) {
    ...
    el_blockhead_t *new_block = PTR_PLUS_BYTES(new_footer, sizeof(el_blockfoot_t));
    new_block->size = remaining_size;
    new_block->state = EL_AVAILABLE;
    el_blockfoot_t *new_block_footer = el_get_footer(new_block);
...
void *el_malloc(size_t nbytes) {
    el_blockhead_t *block = use_best_fit ? el_find_best_avail(nbytes) : el_find_first_avail(nbytes);
    if (block == NULL) {
        return NULL;
    }
    el_remove_block(el_ctl.avail, block);
    el_blockhead_t *remaining_block = el_split_block(block, nbytes);
...
void el_merge_block_with_above(el_blockhead_t *lower) {
    ...
    el_blockfoot_t *new_footer = el_get_footer(lower);
    new_footer->size = lower->size;
