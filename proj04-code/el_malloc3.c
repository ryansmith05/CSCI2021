...
el_blockhead_t *el_block_below(el_blockhead_t *block) {
    ...
    el_blockhead_t *below_head = el_get_header(below_foot);
    if ((void *)below_head < el_ctl.heap_start) {
        return NULL;
    }
    return below_head;
}
...
el_blockhead_t *el_split_block(el_blockhead_t *block, size_t new_size) {
    ...
    el_blockhead_t *new_block = PTR_PLUS_BYTES(new_foot, sizeof(el_blockfoot_t));
    new_block->size = remaining_size;
    new_block->state = EL_AVAILABLE;

    el_blockfoot_t *new_block_foot = el_get_footer(new_block);
    new_block_foot->size = remaining_size;
